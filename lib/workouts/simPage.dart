...
import 'package:zilch_workout/workouts/simPainter.dart';
...
class _SimPageState extends State<SimPage> {
  ...
  void _changeSlope(double amt) {
    _currentSlope += amt;
    _currentSlope = _currentSlope < -20.0 ? -20.0 : _currentSlope;
    _currentSlope = _currentSlope > 20.0 ? 20.0 : _currentSlope;
    _currentSlope = (_currentSlope * 10).round() / 10;
  }
    ...
    _timer = Timer.periodic(Duration(milliseconds: 500), (timer) {
      if (!_paused) {
        setState(() {
          ...
          if (_currentSlope != _oldSlope) changeTrainerSim();
        });
      }
    });
    _started = true;
  ...
  void _saveWorkout(
    ...
    String dataName = state.user.id + '_SIM_$_startTime';
    int averagePower = 0;
  ...
  Widget build(BuildContext context) {
    ...
    Widget _controlRow() {
      return Row(mainAxisAlignment: MainAxisAlignment.spaceAround, children: [
        ...
        Text(_currentSlope.toStringAsFixed(1) + ' %', style: largeText),
        GestureDetector(
            onTap: () {
              setState(() {
                _changeSlope(0.1);
    ...
    return BlocProvider(
      create: (context) => WorkoutBloc(
        ...
        ..add(UpdateWorkoutName('SIM_FREE')),
